...
- **Authentication**: NextAuth v5 + Argon2id Password Hashing + 2FA TOTP (`otplib`) + Enkripsi AES-256-GCM untuk Kunci TOTP
...
6. **Proteksi Admin & Enkripsi TOTP**:
   ...
   - **Kunci Rahasia TOTP 2FA di-enkripsi menggunakan algoritma AES-256-GCM** sebelum disimpan di database (tidak pernah disimpan dalam bentuk plaintext).
...
### 1. Inisialisasi Database & Seed Admin
...
### 2. Pendaftaran Akun iPaymu (Merchant QRIS)
...
3. Salin `VA` dan `API Key` dari dashboard iPaymu ke Environment Variables.
...
- **WhatsApp**: Diatur via environment variable `NEXT_PUBLIC_WHATSAPP_NUMBER`
- **Email**: Diatur via environment variable `NEXT_PUBLIC_CONTACT_EMAIL`
